  ...
  clicked() {
    ...
    this.props.items
      .updateOne(
        ...
        { $set: { done: !this.props.item.done } }
      )
      .then(() => this.props.onChange());
  }

  render() {
    let itemClass = this.props.item.done ? "done" : "";
    return (
...
var TodoList = class extends React.Component {
  loadList() {
    ...
    let obj = this;
    this.items.find({}, {}).execute().then(function(data) {
  ...
  checkHandler(id, status) {
    this.items.updateOne({ _id: id }, { $set: { done: status } }).then(() => {
      this.loadList();
    }, { rule: "done" });
  }
  ...
  clear() {
    this.setState({ requestPending: true });
    this.items.deleteMany({ done: true }).then(() => {
      this.loadList();
    });
  }
    ...
    let loggedInResult = (
      <div>
        <div className="controls">
          ...
          />
          {this.state.items.filter(x => x.done).length > 0
            ? <div
                href=""
...
stitchClientPromise.then(stitchClient => {
  let db = stitchClient.service("mongodb", mongodbService).db("pick2do");
